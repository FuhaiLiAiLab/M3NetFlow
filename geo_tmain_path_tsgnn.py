...
def build_geotsgnn_model(args, device, dataset):
    ...
    model = TSGNNDecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, decoder_dim=args.decoder_dim,
                num_gene=num_gene, node_num=node_num, num_edge=num_edge, num_gene_edge=num_gene_edge, device=device)
...
def train_geotsgnn_model(dataset_loader, adj, edge_feat, edge_adj_index, model, device, args, learning_rate, dataset):
    ...
    for batch_idx, data in enumerate(dataset_loader):
        ...
        ### [adj, edge_feat, edge_adj_index]
        # import pdb; pdb.set_trace()
        adj = Variable(adj.float(), requires_grad=False).to(device)
        edge_feat = Variable(edge_feat.float(), requires_grad=False).to(device)
        edge_adj_index = Variable(edge_adj_index.long(), requires_grad=False).to(device)
        
        ypred = model(x, edge_index, drug_index, adj, edge_feat, edge_adj_index, dataset)
...
def train_geotsgnn(args, fold_n, load_path, iteration_num, device, dataset):
    ...
    # READ [adj, edge_feat, edge_adj, trans, edge_index] FILES 
    ...
    edge_feat = torch.from_numpy(np.load(form_data_path + '/edge_feat.npy') ) 
    edge_adj = sparse_mx_to_torch_sparse_tensor(sparse.load_npz(form_data_path + '/edge_adj_sparse.npz'))
    edge_adj_index = edge_adj._indices()
    trans = sparse_mx_to_torch_sparse_tensor(sparse.load_npz(form_data_path + '/trans_sparse.npz'))


    # BUILD [WeightBiGNN, DECODER] MODEL
    model = build_geotsgnn_model(args, device, dataset)
    if args.model == 'load':
    ...
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            model, batch_loss, batch_ypred = train_geotsgnn_model(dataset_loader, adj, edge_feat, edge_adj_index, model, device, args, learning_rate, dataset)
        ...
        # fold_n = 1
        test_save_path = path
        test_pearson, test_loss, tmp_test_input_df = test_geotsgnn(prog_args, fold_n, model, test_save_path, device, dataset)
...
def test_geotsgnn_model(dataset_loader, adj, edge_feat, edge_adj_index, model, device, args, dataset):
    batch_loss = 0
    for batch_idx, data in enumerate(dataset_loader):
...
def test_geotsgnn(args, fold_n, model, test_save_path, device, dataset):
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        print('TEST MODEL...')
        # import pdb; pdb.set_trace()
        model, batch_loss, batch_ypred = test_geotsgnn_model(dataset_loader, adj, edge_feat, edge_adj_index, model, device, args, dataset)
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
...
if __name__ == "__main__":
    ...
    # load_path = './' + dataset + '/result/epoch_100/best_train_model.pt'
    load_path = ''
    yTr = np.load('./' + dataset + '/form_data/yTr' + str(fold_n) + '.npy')
    dl_input_num = yTr.shape[0]
    epoch_iteration = int(dl_input_num / prog_args.batch_size)
    start_iter_num = 100 * epoch_iteration
    train_geotsgnn(prog_args, fold_n, load_path, start_iter_num, device, dataset)

    # ### TEST THE MODEL
    # # TEST [FOLD-1]
    # fold_n = 1
    # model = build_geotsgnn_model(prog_args, device, dataset)
    # test_load_path = './' + dataset + '/result/tsgnn/epoch_60/best_train_model.pt'
    # model.load_state_dict(torch.load(test_load_path, map_location=device))
    # test_save_path = './' + dataset + '/result/tsgnn/epoch_60'
    # test_geotsgnn(prog_args, fold_n, model, test_save_path, device, dataset)
