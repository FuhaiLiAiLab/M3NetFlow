...
def edge_index_dataloader():
    ...
    geo_datalist = read_batch(0, 1, xTr, yTr, drugTr, num_feature, num_gene, num_drug, edge_index, dataset)
...
def build_geopna_model(args, device, dataset):
    ...
    # [deg_histogram]
    deg_histogram = edge_index_dataloader()
    # BUILD UP MODEL
    model = PNADecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, 
                decoder_dim=args.decoder_dim, deg_histogram=deg_histogram, device=device)
...
def train_geopna(args, fold_n, load_path, iteration_num, device, dataset):
    # TRAINING DATASET BASIC PARAMETERS
    # [num_feature, num_gene, num_drug]
    num_feature = 8
    dict_drug_num = pd.read_csv('./' + dataset + '/filtered_data/drug_num_dict.csv')
    ...
    model = build_geopna_model(args, device, dataset)
    if args.model == 'load':
    ...
    for i in range(1, epoch_num + 1):
        ...
        # fold_n = 1
        test_save_path = path
        test_pearson, test_loss, tmp_test_input_df = test_geopna(prog_args, fold_n, model, test_save_path, device, dataset)
...
def test_geopna_model(dataset_loader, model, device, args):
    batch_loss = 0
    for batch_idx, data in enumerate(dataset_loader):
...
def test_geopna(args, fold_n, model, test_save_path, device, dataset):
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        print('TEST MODEL...')
        # import pdb; pdb.set_trace()
        model, batch_loss, batch_ypred = test_geopna_model(dataset_loader, model, device, args)
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
...
if __name__ == "__main__":
    ...
    train_geopna(prog_args, fold_n, load_path, start_iter_num, device, dataset)

    # ### TEST THE MODEL
    # # TEST [FOLD-1]
    # fold_n = 1
    # model = build_geopna_model(prog_args, device, dataset)
    ...
    # test_geopna(prog_args, fold_n, model, test_save_path, device, dataset)