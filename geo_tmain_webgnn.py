...
if __name__ == "__main__":
    ...
    # load_path = './' + dataset + '/result/epoch_200/best_train_model.pt'
    load_path = ''
    yTr = np.load('./' + dataset + '/form_data/yTr' + str(fold_n) + '.npy')
    ...
    # # TEST [FOLD-1]
    # fold_n = 1
    # model = build_geowebgnn_model(prog_args, device, dataset)
    # test_load_path = './' + dataset + '/result/epoch_50/best_train_model.pt'
    # model.load_state_dict(torch.load(test_load_path, map_location=device))
    # test_save_path = './' + dataset + '/result/epoch_50'
    # test_geowebgnn(prog_args, fold_n, model, test_save_path, device, dataset)
