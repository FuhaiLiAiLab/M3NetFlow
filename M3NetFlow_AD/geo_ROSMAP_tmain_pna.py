...
from enc_dec.geo_pna_decoder import PNADecoder
...
def edge_index_dataloader(graph_output_folder):
    ...
    geo_datalist = read_batch(0, 1, xTr, yTr, num_feature, node_num, edge_index, graph_output_folder)
    dataset_loader, node_num, feature_dim = GeoGraphLoader.load_graph(geo_datalist, prog_args)
    deg_histogram = get_degree_histogram(dataset_loader)
    return deg_histogram

def build_geopna_model(args, device, graph_output_folder, num_class):
    print('--- BUILDING UP PNA MODEL ... ---')
    # GET PARAMETERS
    ...
    node_num = len(gene_name_list)
    # [deg_histogram]
    deg_histogram = edge_index_dataloader(graph_output_folder)
    # BUILD UP MODEL
    model = PNADecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, 
                deg_histogram=deg_histogram, device=device, num_class=num_class, node_num=node_num)
...
def train_geopna_model(dataset_loader, model, device, args, learning_rate):
...
def train_geopna(args, fold_n, load_path, iteration_num, device, graph_output_folder, dataset, num_class):
    # TRAINING DATASET BASIC PARAMETERS
    # [num_feature, num_gene, num_drug]
    num_feature = 10
    final_annotation_gene_df = pd.read_csv(os.path.join(graph_output_folder, 'map-all-gene.csv'))
    ...
    model = build_geopna_model(args, device, graph_output_folder, num_class)
    if args.model == 'load':
    ...
    path = './' + dataset + '-result/pna/%s' % (folder_name)
    unit = 1
    while os.path.exists('./' + dataset + '-result/pna/') == False:
        os.mkdir('./' + dataset + '-result/pna/')
    while os.path.exists(path):
        path = './' + dataset + '-result/pna/%s_%d' % (folder_name, unit)
        unit += 1
    os.mkdir(path)
    ...
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            model, batch_loss, batch_ypred = train_geopna_model(dataset_loader, model, device, args, learning_rate)
        ...
        # fold_n = 1
        test_save_path = path
        test_acc, test_loss, tmp_test_input_df = test_geopna(prog_args, fold_n, model, test_save_path, device, graph_output_folder, i)
...
def test_geopna(args, fold_n, model, test_save_path, device, graph_output_folder, i):
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
...
if __name__ == "__main__":
    ...
    for fold in range(1, 6):
        ...
        train_geopna(prog_args, fold_n, load_path, start_iter_num, device, graph_output_folder, dataset, num_class)