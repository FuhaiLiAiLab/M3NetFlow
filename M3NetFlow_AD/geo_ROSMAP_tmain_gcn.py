...
def build_geogcn_model(args, device, graph_output_folder, num_class):
...
def train_geogcn(args, fold_n, load_path, iteration_num, device, graph_output_folder, dataset, num_class):
    ...
    model = build_geogcn_model(args, device, graph_output_folder, num_class)
    if args.model == 'load':
    ...
    path = './' + dataset + '-result/gcn/%s' % (folder_name)
    unit = 1
    while os.path.exists('./' + dataset + '-result/gcn/') == False:
        os.mkdir('./' + dataset + '-result/gcn/')
    while os.path.exists(path):
        path = './' + dataset + '-result/gcn/%s_%d' % (folder_name, unit)
        unit += 1
    os.mkdir(path)
...
if __name__ == "__main__":
    ...
    for fold in range(1, 6):
        ...
        train_geogcn(prog_args, fold_n, load_path, start_iter_num, device, graph_output_folder, dataset, num_class)