...
from enc_dec.geo_mixhop_decoder import MixhopDecoder
...
def build_geomixhop_model(args, device, graph_output_folder, num_class):
    print('--- BUILDING UP MixHop MODEL ... ---')
    # GET PARAMETERS
    ...
    # BUILD UP MODEL
    model = MixhopDecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, 
                num_head=args.num_head, device=device, num_class=num_class, node_num=node_num)
...
def train_geomixhop_model(dataset_loader, model, device, args, learning_rate):
...
def train_geomixhop(args, fold_n, load_path, iteration_num, device, graph_output_folder, dataset, num_class):
    ...
    model = build_geomixhop_model(args, device, graph_output_folder, num_class)
    if args.model == 'load':
    ...
    path = './' + dataset + '-result/mixhop/%s' % (folder_name)
    unit = 1
    while os.path.exists('./' + dataset + '-result/mixhop/') == False:
        os.mkdir('./' + dataset + '-result/mixhop/')
    while os.path.exists(path):
        path = './' + dataset + '-result/mixhop/%s_%d' % (folder_name, unit)
        unit += 1
    os.mkdir(path)
    # import pdb; pdb.set_trace()
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            model, batch_loss, batch_ypred = train_geomixhop_model(dataset_loader, model, device, args, learning_rate)
        ...
        # fold_n = 1
        test_save_path = path
        test_acc, test_loss, tmp_test_input_df = test_geomixhop(prog_args, fold_n, model, test_save_path, device, graph_output_folder, i)
...
def test_geomixhop_model(dataset_loader, model, device, args):
    batch_loss = 0
    for batch_idx, data in enumerate(dataset_loader):
...
def test_geomixhop(args, fold_n, model, test_save_path, device, graph_output_folder, i):
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        print('TEST MODEL...')
        model, batch_loss, batch_ypred = test_geomixhop_model(dataset_loader, model, device, args)
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
...
if __name__ == "__main__":
    ...
    for fold in range(1, 6):
        ...
        train_geomixhop(prog_args, fold_n, load_path, start_iter_num, device, graph_output_folder, dataset, num_class)